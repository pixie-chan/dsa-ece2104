...
- **Revision this list was verified at:** `sha256 cbc321cd4a017fb8`, 231128 bytes, mtime 2026-09-19 00:09:38 IST. Every item below is applied there and the health battery passes 15 of 15 rows.
- **Live revision since:** `sha256 5445084fffe115f6`, 231889 bytes, mtime 00:16:35 IST. It adds one anchor inside the topic plan section, the cross-link to the topic-wise explanation set, and nothing else (snapshot `versions/v6-pre-topicwise-link/`). The battery was re-run against it: health 15 of 15, leftovers probe 8 of 8, token gate clean, no overflow at either width.
