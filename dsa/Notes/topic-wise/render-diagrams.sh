...
#   bash ~/Documents/MUJ-SEM-3/DSA/Notes/topic-wise/render-diagrams.sh
#
...
set -u
DIAG="$HOME/Documents/MUJ-SEM-3/DSA/Notes/topic-wise/diagrams"
CFG=/tmp/kiki-mmdc-puppeteer.json
TMP=/tmp/topic-wise-svg
mkdir -p "$TMP"
cat > "$CFG" <<'JSON'
...
PY
  ...
  if $MMDC -i "$f" -o "$base.png" -b white -w 1400 -s "$SCALE" -p "$CFG" >/tmp/mmdc.log 2>&1; then
    printf '%-44s natural %sx%-6s scale %-8s -> %s\n' "$f" "$W" "$H" "$SCALE" "$base.png"
  else
...
d = os.path.expanduser("~/Documents/MUJ-SEM-3/DSA/Notes/topic-wise/diagrams")
