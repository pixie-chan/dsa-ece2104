...
`lec05_run_output.txt`, `lec11_15_run_output.txt`, `lec16_recursion_output.txt`,
`lec16_18_sorts_run_output.txt`, `lec19_21_trees_run_output.txt`, `lec22_analysis_run_output.txt`
and `lec23_formulations_run_output.txt`. Nothing in this file is typed from memory; the seven
captures and the lesson programs that produced them are the evidence.
...
| Answer keys | last section of this file, one key per topic, numbered to match the questions one for one |
...
Keys are here, separate from the questions above. Each key carries the same number as the question
it answers, so key 3 answers question 3 of the same topic. Derived arithmetic is marked as derived;
everything else is quoted from a capture.
