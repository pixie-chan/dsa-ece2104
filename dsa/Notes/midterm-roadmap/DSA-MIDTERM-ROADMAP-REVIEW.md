...
# Round 20, final check at revision 2131627215

Checked at 2026-09-18 23:52 IST against `sha256 2131627215d8df38` (230493 bytes, mtime 23:29:35), the last save of the session.

## Everything measurable passes

- All three inline scripts parse.
- `health.mjs`: 15 of 15 PASS, including the navigator handshake, the trail numbering, the spine ranges, the timeline gap, the rail wheel chaining and the OS dark preference.
- `sweep.mjs`, eleven widths from 320px to 1920px: no overflow anywhere, rail `sticky` down to 1040px and `static` below it, slab columns 7 then 2 then 1, no strip wheel hijack.
- `wheel.mjs`: 1200px of page travel over the pinned bar against 1200px over the content.
- `verify4.mjs`: `beforeprint` opens 12 answers, `afterprint` restores 2, and a double `beforeprint` leaves 2 open instead of 12.
- Print: 84 pages, answer bodies present, `SHOW ANSWER` count 0.

## Batch 2 closed out

- B2 fixed: Block F reads `15 lectures, past the spine`, its fill bar is gone, and `SLABS` now carries the `f` entry, so nothing is left unpainted.
- B3 fixed: the chips read `topic plan`, `inventory`, `honesty`.
- B6 fixed: all ten figures moved into `diagrams/` beside the page and every referenced file exists, so the dossier is self-contained again.
- B7 fixed: zero mangled struct literals. `sizeof(Node{int data; Node* next;})` and the three field tree node are back.
- B5 nearly: 69 of 69 tables now carry a caption and 68 of 69 are wrapped. One table and about seven `<th>` attributes remain.
- B8 fixed, and the section renumbering in the prose (`section 04, the chain`, `section 11, the numericals`, `section 13, the evidence`) matches the chip order, which I checked against the nav.

## What is left, in order

1. B4b: the meta description ends `each with its own answer key.re still open.`, a clipped join. Exact replacement in the apply list.
2. B5 residue: wrap the "four gaps" table, add `scope="col"` to about seven headers.
3. B9: the duplicate `.slab__link{min-width:44px}` declaration.

That is three cosmetic items on a page that now passes every behavioural check I have.

---

# Round 21, the three leftovers closed at revision 19f35017

Checked at 2026-09-19 00:00 IST against `sha256 19f350175c8da8ba` (230529 bytes, mtime 2026-09-18 23:56:34), the state after the Batch 2 leftovers were applied. Pre-edit state preserved at `versions/v4-pre-leftovers/` (`2131627215d8df38`, 230493 bytes).

## What was changed

Four literal edits, three items, nothing reformatted:

| # | Location | Before | After |
|---|---|---|---|
| B4 | line 17 | `... each with its own answer key.re still open.` | `... each with its own answer key, and the gaps that are still open.` |
| B5 | line 700 | `<h3>The four gaps that only practice closes</h3><table>` | `</h3><div class="tbl-wrap"><table>` |
| B5 | line 700 | that table's closing `</table>` | `</table></div>` |
| B9 | line 292 | `.slab__link{min-width:44px} .slab__link{position:absolute; ...}` | `.slab__link{min-width:44px; position:absolute; ...}` |

The B4 replacement keeps the existing front half of the description and repairs only the clipped tail, so the wording the page already had is preserved. The B9 merge puts `min-width:44px` first, which keeps the declaration order identical to the two-rule form.

## The one reported defect that did not exist

Round 20 and both B5 entries in the apply list carried "about seven `<th>` cells still lack `scope`". Counted over the DOM, there were zero: 234 `<th>` elements, 227 of them `<th scope="col">` and 7 of them `<th class="n" scope="col">`. Every one carried the attribute.

The count came from `grep -o '<th' file | wc -l`, which returns 277. That pattern matches `<thead>` as well, and the file has 43 of those, so 277 minus 234 is the `<thead>` opening tags. Corrected census:

```
grep -o '<table' file | wc -l                # 69
grep -o 'class="tbl-wrap"' file | wc -l      # 69
grep -o '<th[^>]*>' file | sort | uniq -c    # 227 plain, 7 class="n", every line carries scope="col"
```

The lesson is in the command shape, not in the page: a substring count over markup will match a longer tag that shares the prefix. Counting element *kinds* is a census (`sort | uniq -c` on the full tag), which cannot be fooled that way and also shows the variants.

## How each claim was measured

- **B4** read from the DOM, `document.querySelector('meta[name="description"]').content`, 428 characters, checked for whole sentences and for a `word.word` join pattern. Source grep agrees: `/key\.re/` is gone from the file.
- **B5** counted both in the source and in the DOM: 69 `<table`, 69 `class="tbl-wrap"`, 69 `</table></div>`, and in the DOM 69 tables with 0 outside a `.tbl-wrap`. The wrapped table was located by its caption text, then its `.tbl-wrap` parent was confirmed present, `overflow-x` computed `auto`, and document overflow at 390px measured at 0.
- **B9** read out of the CSSOM, not the source: one rule whose `selectorText` is exactly `.slab__link`, `cssText` starting `min-width: 44px; position: absolute; inset: 0px; ...`, with `getComputedStyle` still reporting `min-width: 44px` and the link box 356 x 235 at 390px, so the merge kept the behaviour as well as the intent.
- **Print** re-run with `--print-to-pdf` at the new revision: 83 pages, `SHOW ANSWER` count 0, and all four rows of the newly wrapped table present as text (`Timed arithmetic`, `Sit the mock paper in one 60 minute block`, `Draw each traversal and each delete case`, `Cover the tags, read a question`), so the wrap did not clip the table on paper.

## Full battery, same revision

- All three inline scripts: `node --check` clean.
- `health.mjs`: 15 of 15 PASS.
- `leftovers.mjs` (new, the probe for this round): 8 of 8 PASS, 0 page errors.
- `verify14.mjs`: trail reads 1 through 10, `s7Spine` 16, `errs` empty.
- `wheel.mjs`: page 2500 to 2900 across the pinned bar, strip stays at 0.
- `verify4.mjs`: `beforePrintOpen` 2, `double` 2 (equal), `openAfterPdf` 0.
- `sweep.mjs`: no overflow at any of ten widths, 320px to 1920px.
- `export-tokens.py --check`: `clean`, rc 0, 46 properties.

## Probe pitfall worth keeping

The first version of `leftovers.mjs` reported "zero `.slab__link` rules" and read like the declaration had been deleted. It had not. Current Chromium gives every `CSSStyleRule` a `cssRules` property, and it exists but is empty, so a recursion guard written as `if (rule.cssRules) return [...rule.cssRules].forEach(walk)` recurses into nothing and never inspects the rule. The guard has to be `if (rule.cssRules && rule.cssRules.length)`. Any CSSOM sweep written before nested CSS existed will silently report zero matches.
