...
## Projected MTE scope (added 2026-09-18)

The handout and the classroom list disagree, and the list wins for the paper. Evidence is in the
HTML page, section 03: the handout's `MID SEMESTER EXAMINATION` row sits on page 5 between the
lecture 16 row and the lecture 17 row, and every tree, heap, sort, hashing and graph row from
17 to 37 carries the `End Term` tag. The lecture plan never schedules asymptotic notation at all
(one occurrence of the word complexity, in the page 1 course description).

The fourteen projected topics, and where each one is taught here:

| Topic | Handout | State |
|---|---|---|
| Array | lectures 1 to 4 | already covered |
| Pointers | lectures 3 to 5 | already covered |
| Stack by Array | lecture 11 | already covered |
| Queue | lectures 13 to 15 | already covered |
| Linked List | lectures 6 to 8 | already covered |
| Merge Sort | lecture 27, End Term | new, lesson 17 |
| Quick Sort | lecture 29, End Term | new, lesson 18 |
| Algorithm fundamentals | spread through the plan | new, lesson 22 |
| Asymptotic notation | no row, never scheduled | new, lesson 22 |
| Finding Complexity | no row, never scheduled | new, lesson 22 |
| Trees fundamentals | lecture 17, End Term | new, lesson 19 |
| Formulation in C | implied by every implementation row | new, lesson 23 |
| Tree types and BST | lectures 18 to 19, End Term | new, lessons 20 and 21 |
| Different Operations | implied by the session outcomes | new, the operations matrix |

New in the HTML page: the scope conflict with the handout evidence, a fifteen rung session plan
covering all fourteen topics plus the timed mock, the sorting, analysis, trees and formulation
modules with every count measured, the operations matrix, twenty four new numeric problems with a
separate key, and a 40 question 60 minute mock paper with its key in its own section.

Measurements: lessons 16 to 18 (sorting), 19 to 21 (trees and BST), 22 (analysis) and 23
(formulation), captured in `learning/cpp/exercises/lec16_18_sorts_run_output.txt`,
`lec19_21_trees_run_output.txt`, `lec22_analysis_run_output.txt` and
`lec23_formulations_run_output.txt`. All eight lessons build with zero warnings under
`-Wall -Wextra -Wpedantic -std=c++17 -O2`.

Verification: `python3 ~/learning/cpp/exercises/verify_mte_guide.py` (140 checks, 0 failures, with
a mutation battery), the APPLY list fixes A1 to A11, `node ~/.cache/roadmap-qa/health.mjs`
(15 of 15 rows PASS), and the UI gate.

Sources for the topics the handout never schedules: Kanetkar (C formulation and pointers),
Drozdek and Weiss (trees and BST operations), Goodrich (algorithm analysis and certificates),
CLRS (sorting bounds and counting).
