...
- **Pinned revision:** `sha256 e5b32c74af252811`, 1317 lines, 90107 bytes, mtime 2026-09-18 16:52:54 IST
- **Status at this revision:** P0, P0b and P2 are fixed. P1 is the only remaining blocker: the section navigator still never runs.
...
2. If the hash is not `e5b32c74af252811`, do not trust the line numbers. Locate each edit by the quoted snippet instead.
3. Fix P1 before anything else. Until then the section navigator never executes, so chip state, `prev`/`next` and the counter cannot be tested at all.
...
Print check without a browser dialog:

```
cd /tmp && ~/.cache/ms-playwright/chromium-1234/chrome-linux64/chrome --headless --disable-gpu --no-sandbox --print-to-pdf=/tmp/p.pdf --no-pdf-header-footer file://$HOME/Documents/MUJ-SEM-3/DSA/Notes/midterm-roadmap/DSA-MIDTERM-ROADMAP.html && pdftotext /tmp/p.pdf - | grep -c "allocator bookkeeping"
```
...
| P12 | Content | The markdown twin says "the ten numerical problems" |
...
## P4, Medium: `og-card.png` does not exist

**Where:** line 23, `<meta property="og:image" content="og-card.png">`. The folder contains `og-card.svg` only.

**Edit, pick one:**

1. Generate the file. Verified on this machine: `inkscape og-card.svg -w 1200 -h 630 -o og-card.png` produces a 1200x630 PNG (108 KB). The card uses `Georgia` and generic `monospace`, so it renders the same without the Instrument fonts installed.
2. Point the meta back at a file that exists and accept that most platforms do not render SVG.

**Related:** on the host where the page is published, `og:image` must be an absolute URL, and `og:url` (line 22) is the repo root (`https://github.com/pixie-chan/dsa-ece2104`, resolves 200), not the page URL if the HTML sits in a subfolder such as `Notes/midterm-roadmap/`.

**Verify:** `ls -la og-card.*` shows the referenced file, and `identify og-card.png` reports 1200x630.
...
- **Unused custom properties**, declared and never read: `--dur-1`, `--hair`, `--rule`, `--s-1`, `--shadow-2`, `--w-reg`, `--w-med`, `--w-semi`. Either delete them or use them; `tokens.css` in the same folder still declares them.
...
- **Token drift:** `--text-2xl` is `2.5rem` in the page and `2.25rem` in `tokens.css`; `--text-3xl` and `--shadow-2` also differ. Pick one source of truth.

---

## P12, Content: the markdown twin

**Where:** `DSA-MIDTERM-ROADMAP.md:93` says "solve the ten numerical problems in section 6". Section 6 lists twelve and the answer key has twelve rows. The HTML footer calls the markdown "the same content", so change "ten" to "twelve".

---

## P13, Judgment calls, not defects

- **The chip strip hijacks vertical wheel.** The navigator's `wheel` handler walks the strip sideways while it still has room. Measured overflow: 161px at 1460px width, 769px at 375px, so it is active nearly always, and it is the same trap class as P5. The chips are reachable by tab, by prev/next, and natively, so dropping it is defensible.
